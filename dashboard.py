...
st.set_page_config(page_title="金融影像风险检测", page_icon="🔎", layout="wide")
...
if not (OUTPUT / "run_summary.json").exists():
    st.error("尚未生成 MVP 结果，请先运行：python -m mvp.pipeline")
    st.stop()
...
col1.metric("金融影像", summary["total_images"])
col2.metric("面签照片", summary["selected_face_signing"])
col3.metric("分类准确率", f'{metrics["test"]["accuracy"]:.1%}')
col4.metric("测试集 Macro-F1", f'{metrics["test"]["macro_f1"]:.1%}')

tab_overview, tab_classification, tab_search, tab_threshold = st.tabs(
    ["数据概览", "分类结果", "Top-K 风险检索", "阈值实验"]
)

with tab_overview:
    st.subheader("数据统计")
    counts = (
        predictions.groupby(["image_type", "predicted_type"])
        .size()
        .reset_index(name="count")
    )
    st.bar_chart(predictions["predicted_type"].value_counts())
    st.dataframe(counts, use_container_width=True, hide_index=True)
    st.caption(f'模型：{summary["model_name"]} | 运行设备：{summary["device"]}')

with tab_classification:
    st.subheader("SigLIP2 分类与面签筛选")
...
with tab_search:
    ...
    query_id = st.selectbox("查询交易", query_ids)
    ...
    for _, row in matches.iterrows():
        ...
        with right:
            ...
            st.write(f'风险等级：**{row["当前判定"]}**')
            st.write(f'预设风险分层：**{row["risk_level"]}**')
            st.write(f'关联交易：`{row["match_loan_id"]}`')
