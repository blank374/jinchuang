...
class ImageClassifier:
    ...
    def is_sign_photo(self, image_tensor: torch.Tensor, threshold: float = 0.22):
        """判断是否为面签照片

        Args:
            image_tensor: [1, 3, 224, 224] 归一化后的 tensor
            threshold: 面签类别的相似度阈值

        Returns:
            is_sign: bool (当预测类别是面签照片且置信度超过阈值)
            confidence: float (面签类别的相似度分数)
        """
        scores = self.classify(image_tensor)[2]
        sign_score = scores.get("面签照片", 0.0)
        return sign_score > threshold, sign_score
