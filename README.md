...
3. **分层阈值风控**  
   ...
   - 跨客户相似：疑似冒用/套用，要求更高相似度才进入反欺诈复核；
   - 同客户相似：可能是续贷/复用，采用略低阈值进入运营/合规复核；
   - 自身匹配：自动跳过。
...
5. **风险关系图谱创新点**  
   系统在单条 Top-K 相似结果之上进一步构建“贷款-影像相似关系图”，把多笔业务之间的可疑关联串起来，输出综合欺诈分、风险关系簇、节点连接度和跨产品复用标签。这样可以识别两类传统单阈值方案容易漏掉的风险：一是同一影像或相似场景在多个客户之间反复出现，二是同一组相似面签照片跨不同信贷产品复用。
...
| 场景 | 关系 | 阈值 | 风险含义 |
|---|---|---:|---|
| 跨客户命中 | `cross_customer` | 0.95 | 疑似冒用/套用，比同客户重复更严格 |
| 同客户命中 | `same_customer` | 0.92 | 可能是续贷/复用，进入运营/合规复核 |
| 当前可疑交易/高风险 | default/high | 0.97 | 原 MVP 人工审核校准阈值 |
| 中风险候选 | medium | 0.93 | 抽检/二审候选池 |

综合欺诈分并不替代阈值，而是在阈值命中后继续排序复核优先级。评分会综合考虑相似度、超过阈值的幅度、是否跨客户、是否跨产品、是否处于风险关系簇、节点连接度等信号。这样既能保留阈值策略的可解释性，又能把最可能形成批量欺诈的样本排到前面。
